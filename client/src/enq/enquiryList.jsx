...
export default function EnquiryList({ data=[] , getAllEnquiry,setFormData}) {
  ...
  let editRow=(id)=>{
    ...
    axios.get(`http://localhost:8020/api/website/enquiry/single/${id}`)
    .then((res)=>{
      let data = res.data
              ...
              data.map((item,index)=>{
                return(
                  ...
                  <TableCell className="flex">
                    <Button className="bg-red-500 " onClick={() => deleteRow(item._id)} >Delete</Button>
                    <Button color="success"  onClick={()=> editRow(item._id)} className="bg-blue-500 text-amber-50 ml-2">Edit</Button>
                  </TableCell>
                </TableRow>
                )
              })
